...
class ShortAnsDetails{
    ...
    public function StaffShortAnsPostData(){
        ...
        $sql = $conn->prepare("INSERT INTO shortans (CourseID,StaffID,unit,Topic,ques,ans,QuestionType)
            VALUES (?,?,?,?,?,?,?)");

            $sql->bind_param("sssssss",$CourseID,$StaffID,$Unit,$topic,$Ques,$correct,$QuestionType);
            $sql->execute();
            $sql->close();
            $conn->close();
        }
    ...
    public function StaffShortAnsSpecific(){
        ...
        include "db.php";
        $sql = "SELECT StaffID,Topic, id, unit, ques, ans FROM shortans where CourseID = '{$_SESSION['CourseID']}' ORDER BY unit ASC";
        $result = mysqli_query($conn, $sql);
        if (mysqli_num_rows($result) > 0) 
        {
            while($row = mysqli_fetch_assoc($result)) {
    ...
    public function DeleteShortAns()
    {
       ...
       $vari="$vari";
       //echo($vari);
        $sql = "DELETE FROM shortans WHERE id = '{$vari}'";
        //echo($sql);
        $result = mysqli_query($conn, $sql);
        //var_dump($result);
        if (mysqli_num_rows($result) > 0) 
    ...
    public function QuizTopic()
        ...
        public function GetTopicDetails(){
           ...
           $vari="$vari";
            $sql = "SELECT * FROM topic WHERE CourseID = '{$vari}' AND  QuestionType = 'shortans' ";
            $result = mysqli_query($conn, $sql);
            if (mysqli_num_rows($result) > 0) 
            {
                while($row = mysqli_fetch_assoc($result)) {
