...
public function StaffPrivatePostData(){
    ...
    $sql=$conn->prepare("INSERT INTO privatechat (CourseID,StaffID,RegNo,Name,ques)
        VALUES ( ?,?,?,?,?)");

    $sql->bind_param("sssss",$CourseID,$StaffID,$ID,$Name,$Ques);
    $sql->execute();
    $sql->close();
    $conn->close();
    }
    ...
    public function StaffPrivateChatSpecific(){
        ...
        $sql = "SELECT * FROM privatechat where CourseID = '{$_SESSION['CourseID']}' ORDER BY date1 DESC";
        $result = mysqli_query($conn, $sql);
        if (mysqli_num_rows($result) > 0) 
        {
            while($row = mysqli_fetch_assoc($result)) {
...
public function ReplyPrivatePostData(){
    session_start();
