...
class ResourceDetails{
    ...
    public function PostData(){
        ...
        include "db.php";
         ...
         if ($_SERVER["REQUEST_METHOD"] == "POST") { 
            ...
            $topic=$_POST['Topic'];
            $arrayCourseID= $arra[0]['CourseID'];
            $query = "INSERT INTO resources(FilePath,VideoLink,Unit,Topic,CourseID) values('$file','$link','$unit','$topic','$arrayCourseID')";
    ...
    public function GetSpecifcResourceDetails(){
        $GetResource=array();
        include "db.php";
        $vari =$_SESSION['CourseID'];
        $vari="$vari";
        $sql = "SELECT * FROM resources Where CourseID='{$vari}' ORDER BY Unit ASC";
        $result = mysqli_query($conn, $sql);
        if (mysqli_num_rows($result) > 0) 
        {
            while($row = mysqli_fetch_assoc($result)) {
