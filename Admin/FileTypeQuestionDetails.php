...
class FileTypeDetails{
    ...
    public function StaffFileTypePostData(){
        ...
        $sql =$conn->prepare("INSERT INTO filetypequestion (CourseID,StaffID,unit,Topic,ques,QuestionType)
            VALUES (?,?,?,?,?,?)");

            $sql->bind_param("ssssss",$CourseID,$StaffID,$Unit,$topic,$Ques,$QuestionType);
            $sql->execute();
            $sql->close();
            $conn->close();
        }
        public function GetFileTypeDetails(){
            ...
            include "db.php";
            $sql = "SELECT * FROM filetypequestion";
    ...
    public function StaffFileTypeSpecific(){
        ...
        include "db.php";
        $sql = "SELECT StaffID,Topic, id, unit, ques FROM filetypequestion where CourseID = '{$_SESSION['CourseID']}' ORDER BY unit ASC";
        $result = mysqli_query($conn, $sql);
        if (mysqli_num_rows($result) > 0) 
        {
            while($row = mysqli_fetch_assoc($result)) {
...
public function DeleteFileType()
    {
       ...
       $vari="$vari";
       //echo($vari);
        $sql = "DELETE FROM filetypequestion WHERE id = '{$vari}'";
        //echo($sql);
        $result = mysqli_query($conn, $sql);
        //var_dump($result);
        if (mysqli_num_rows($result) > 0) 
